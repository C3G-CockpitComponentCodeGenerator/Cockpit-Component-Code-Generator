function generateGlobalDefinitions(
    firmwareModel
) {
    ...
    lines.push(
        `String authkey = "${firmwareModel.authorId}";`
    );

    lines.push("");

    lines.push(
        `String guid = "${firmwareModel.guid}";`
    );

    return lines.join("\n");
}

function generateInitSection(
    firmwareModel
) {
    ...
    lines.push(
        "    messenger.sendCmdArg(guid);"
    );

    lines.push(
        `    messenger.sendCmdArg("${firmwareModel.deviceName}");`
    );
    ...
    lines.push(
        '    messenger.sendCmdArg("AUTHOR=" + authkey);'
    );
...
function generateIdentifyHandler(
    firmwareModel
) {
...
function generateAttachCallbacks(
    firmwareModel
) {
    ...
    if (
        firmwareModel.outputs.length > 0
    ) {
...
if (hasDisplay) {
    ...
    lines.push(
        "    messenger.attach(7, onDisplayUpdate);"
    );
}
